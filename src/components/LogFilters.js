...
const LogFilters = ({ filters, setFilters, onFileSelect, onClearLogs, allLogs = [] }) => {
  ...
  const [activeFilters, setActiveFilters] = useState(['searchText']);
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          >
            <span className="flex gap-0.5">
              {Object.entries(filters.levels).filter(([,v]) => v).map(([level]) => (
                <span key={level} className={`w-1.5 h-1.5 rounded-full ${levelColors[level]}`} />
              ))}
            </span>
            {getLevelDropdownText()}
            <span className={`text-[10px] text-slate-400 transition-transform ${isLevelDropdownOpen ? 'rotate-180' : ''}`}>▼</span>
          </button>
          ...
          {isLevelDropdownOpen && (
            <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-md shadow-lg z-20 min-w-[140px]">
              {['info', 'error', 'warn', 'debug'].map(level => (
                <label key={level} className="flex items-center gap-2 px-3 py-2 hover:bg-slate-50 cursor-pointer text-xs">
                  <input
                    type="checkbox"
        ...
        <div className="relative" ref={dateRangeRef}>
          ...
          >
            ...
            <span className={`text-[10px] text-slate-400 transition-transform ${isDateRangeOpen ? 'rotate-180' : ''}`}>▼</span>
          </button>
            ...
            <div className="absolute top-full left-0 mt-1 bg-white border border-slate-200 rounded-md shadow-lg z-20 p-3 min-w-[280px]">
              <div className="space-y-3">
                ...
                <div className="flex gap-2 pt-1">
                  <button onClick={clearDateRange} className="flex-1 px-2 py-1 text-[11px] text-slate-500 hover:bg-slate-50 rounded transition-colors">
                    Clear
                  </button>
                  <button onClick={() => setIsDateRangeOpen(false)} className="flex-1 px-2 py-1 text-[11px] bg-slate-800 text-white rounded hover:bg-slate-700 transition-colors">
                    Done
                  </button>
                </div>
              </div>
        ...
        {activeFilters
          ...
          .map(filterKey => {
            ...
            return (
              <div key={filterKey} className="flex items-center gap-1 bg-slate-100 rounded-md pl-3 pr-1 py-1">
                <span className="text-[11px] text-slate-500 font-medium">{filterConfig.label}:</span>
                <input
                  type="text"
        ...
        {pendingFilter && (
          <div className="flex items-center gap-1 bg-blue-50 border border-blue-200 rounded-md pl-3 pr-1 py-1">
            ...
            <input
              ...
              autoFocus
            />
            <button onClick={applyPendingFilter} disabled={!pendingValue.trim()} className="text-blue-600 hover:text-blue-800 text-xs p-0.5 font-medium disabled:opacity-30">
              ↵
            </button>
            <button onClick={cancelPendingFilter} className="text-slate-400 hover:text-red-500 text-xs p-0.5">
              ×
            </button>
          </div>
        )}

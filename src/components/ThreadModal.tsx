...
const ThreadModal = ({ isOpen, threadName, logs, currentLogIndex, onClose }: ThreadModalProps) => {
  ...
  const updateSearchMatches = (term: string) => {
    if (!term) {
      setSearchMatches([]);
      setCurrentMatchIndex(-1);
      return;
  ...
  const navigateSearchMatch = (direction: 'next' | 'prev') => {
    ...
    if (match) {
      document
        .getElementById(`thread-log-${match.index}`)
        ?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    }
  };

  const loadMoreLogs = (direction: 'top' | 'bottom') => {
    const amount = 10;
  ...
  const highlightSearchTerm = (text: string, term: string): ReactNode => {
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onClose}
              className="text-slate-400 hover:text-slate-600 w-7 h-7 flex items-center justify-center rounded-md hover:bg-slate-100 transition-colors"
            >
              ×
            </button>
          </div>
        ...
        {isSearchVisible && (
          <div className="px-5 py-2.5 border-b border-slate-100 flex items-center gap-3 flex-shrink-0">
            <div className="flex-1 relative">
              <svg
                className="w-3.5 h-3.5 absolute left-3 top-1/2 -translate-y-1/2 text-slate-300"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                disabled={
                  searchMatches.length === 0 || currentMatchIndex >= searchMatches.length - 1
                }
                className="w-6 h-6 flex items-center justify-center rounded text-slate-400 hover:bg-slate-100 disabled:opacity-30 text-xs"
              >
                ↓
        ...
        <div ref={modalContentRef} className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-2 bg-slate-50 border-b border-slate-100 text-[11px] text-slate-400 flex items-center gap-4 flex-shrink-0">
            <span>
              Showing {displayStartIndex + 1}–{displayEndIndex + 1} of {logs.length}
            </span>
          ...
          {displayStartIndex > 0 && (
            <button
              ...
              className="w-full py-2.5 text-xs text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-colors border-b border-slate-100"
            >
          ...
          <div className="divide-y divide-slate-50">
            {displayedLogs.map((log, relativeIndex) => {
              ...
              const isSearchMatch =
                searchTerm && searchMatches.some((m) => m.index === actualIndex);
              const isCurrentMatch =
                searchTerm &&
                searchMatches.findIndex((m) => m.index === actualIndex) === currentMatchIndex;
                  ...
                  className={`px-5 py-3 transition-colors ${
                    isCurrentLog
                      ...
                      : isCurrentMatch
                        ...
                        : isSearchMatch
                          ? 'bg-emerald-50/30 border-l-2 border-l-emerald-300'
                          : 'border-l-2 border-l-transparent'
                  }`}
                >
                  <div className="flex items-start gap-3">
                    <div
                      className={`w-1.5 h-1.5 rounded-full mt-1.5 flex-shrink-0 ${getLevelColor(log?.level)}`}
                    />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {isSearchMatch && (
                          <span className="text-[10px] font-medium text-emerald-500">match</span>
                        )}
                      </div>
          ...
          {displayEndIndex < logs.length - 1 && (
            <button
              ...
              className="w-full py-2.5 text-xs text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-colors border-t border-slate-100"
            >
        ...
        <div className="px-5 py-2.5 border-t border-slate-100 flex items-center justify-between flex-shrink-0">
          <div className="flex items-center gap-3 text-[11px] text-slate-400">
            <span className="flex items-center gap-1">
              <kbd className="bg-slate-100 px-1 rounded text-[10px] border border-slate-200 font-mono">
                Ctrl+F
              </kbd>{' '}
              Search
            </span>
